...
def main(args):
    ...
    for i, prompt in enumerate(target_prompts):
        ...
        test_prompts = (
            [f"Image in the style of {prompt.strip()}"] * (args.nsamples//5)+
            [f"Art inspired by {prompt.strip()}"] * (args.nsamples//5)+
            [f"Painting in the style of {prompt.strip()}"] * (args.nsamples//5) +
            [f"A reproduction of art by {prompt.strip()}"] * (args.nsamples//5) +
            [f"A famous artwork by {prompt.strip()}"] * (args.nsamples//5)
        )
