...
def main(args):
    ...
    if args.prior_path is not None:
        with open(args.prior_path, 'r') as f:
            prior_list = f.readlines()
    ...
    for epoch in range(args.num_epochs):
        ...
        selected_forget = random.choices(
            list(forget_list.items()), k=args.fnconcepts
        )
        selected_retain = random.choices(
            list(retain_list.items()), k=args.rnconcepts
        )

        if len(prior_list) > 0:
            selected_prior = random.choices(
                prior_list, k=args.rnconcepts
            )
            ...
            if epoch == 0 or epoch < SWITCH_EPOCH:
                ...
                with args.autocast():
                    tmp_forget_samples, images = sample_multiconcept_epoch(
                        pipeline, args, concept, anchor_concept, device, neg_prompt, forget_set=True
                    )
        ...
        for concept, anchor_concept in selected_retain:
            ...
            with args.autocast():
                ...
                samples, _ = sample_multiconcept_epoch(
                    pipeline, args, concept, anchor_concept, device, neg_prompt
                )
                pipeline.enable_lora()

            #     _, images = sample_multiconcept_epoch(
            #         pipeline, args, concept, anchor_concept, device, neg_prompt
            #     )
            # if samples:
            #     wandb.log({
        ...
        # ─── RETAIN LOSS (weight = 0.1) ───────────────────────
        prior_loss = 0
        for concept in selected_prior:
            unet.eval()
            with args.autocast():
                pipeline.disable_lora()
                samples, _ = sample_multiconcept_epoch(
                    pipeline, args, concept, concept, device, neg_prompt
                )
                pipeline.enable_lora()
            ...
            unet.train()
            for sample in samples:
                prior_loss += train_multiconcept_eraseflow_step(
                    sample,
                    unet,
                    pipeline,
                    optimizer,
